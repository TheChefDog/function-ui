...
function AddCaptchaModal({visible, closeModal, openModal, onAddCaptchaHarvester}) {
    const [harvesterName, setHarvesterName] = useState('');
    const [harvesterType, setHarvesterType] = useState('All');
    ...
    const saveCaptchaHarvester = () => {
        ...
        if (harvesterName.length > 0 && harvesterType !== '' && !alreadyExists) {
            const format = {
                name: harvesterName,
                type: harvesterType,
                proxy: harvesterProxy
            };
            mainConfig.addCaptchaHarvester(format);
...
export default observer(AddCaptchaModal);
